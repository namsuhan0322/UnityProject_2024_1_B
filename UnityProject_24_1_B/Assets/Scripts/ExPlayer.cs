...
{
    public Rigidbody rigid;             // 리지드 바디를 소스로 사용하게 받아 온다.
    ...
    void Update()
    {
        ...
        {
            rigid.AddForce(transform.up * Force);           // transform.up(위쪽 방향)으로 리지드 바디에 Force 힘만큼 물리의 힘을 준다.
        }
    ...
    private void OnCollisionEnter(Collision collision)      // 충동이 시작되었을때
    {
        if (collision != null)                              // 충돌 물체가 없지 않을 경우 [!= 다를 경우]
        {
            point = 0;                                      // 충돌이 일어났을때 포인트를 0으로 해준다.
            gameObject.transform.position = new Vector3(0.0f, 3.0f, 0.0f);  // 충돌했을때 위치 초기화
